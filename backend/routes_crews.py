...
async def _load_enrich_context(
    ...
    for c in all_crews:
        if c.id == crew.id:
            continue
        summary = {"id": c.id, "name": c.name, "district": c.district or ""}
...
async def enrich_crew_preview(
    ...
    if parsed is None or not isinstance(parsed, dict):
        return CrewEnrichPreviewResponse(ok=False, ai_provider=provider.name, ai_model=payload.model or "", raw=raw)
    ...
    def normalize_relations(items: list) -> list[CrewEnrichRelationSuggestion]:
        ...
        for it in items[:6]:
            ...
            if rt not in {"rival", "hostile", "allied", "business", "neutral"}:
                rt = "rival"
            out.append(CrewEnrichRelationSuggestion(
                crew_id=cid,
...
async def enrich_crew_apply(
    ...
    added_rels = 0
    updated_rels = 0
    for suggestion in list(payload.apply_rivalries) + list(payload.apply_allies):
        other = await session.get(Crew, suggestion.crew_id)
        if not other or other.id == crew.id:
            continue
        existing_q = await session.execute(select(CrewRelation).where(
            ((CrewRelation.crew_a_id == crew.id) & (CrewRelation.crew_b_id == other.id))
            | ((CrewRelation.crew_a_id == other.id) & (CrewRelation.crew_b_id == crew.id))
        ))
